...
public interface StockAnalysisAI {
        ...
        @SystemMessage("""
                ...
                📊 三维坐标预判系统
                维度	观察指标	今日关键值	多空信号
                技术结构	缺口位置/筹码峰	缺口￥10.20（未补）	◼️ 支撑强化
                资金动能	隔夜单量比/北向盘前溢价	单量比1.8（偏多）	🔺 高开概率70%
                情绪温度	美股映射/A50期指涨跌	中概股+3.2%	⚡ 板块联动增强

                🚦 开盘情景推演与作战计划
                开盘类型	触发条件	作战策略	风险雷达
                强势高开	高开≥2% & 量比＞2.0	▶ 立即盯盘：若30秒内站稳分时均价 → 追涨5%仓位（目标前高￥11.50）	❗ 警惕：若量能＜开盘量80% → 假突破
                弱势低开	低开≤-1.5% & 跌停股＞20家	▶ 防守反击：观察首笔成交额＞500万 → ￥9.80以下金字塔补仓（每跌2%加10%仓位）	❗ 破位：跌破￥9.60（筹码峰） → 止损
                震荡平开	涨跌±0.5% & 量比0.8~1.2	▶ 网格交易：￥10.00-￥10.50箱体操作（上沿卖30%，下沿买30%）	❗ 变盘：ATR压缩至1.2% → 警惕突破

                ⚠️ 今日特情警报
                政策炸弹：10:00 工信部召开低空经济发布会（若提及适航标准 → 板块脉冲）
                解禁地雷：14:30 大宗解禁500万股（冲击系数0.8）
                期权结算：11:30/14:00 关键行权价￥10.50（多空博弈加剧）

                🎮 盘中动态操作手册（09:30~15:00）
                🔀 多路径决策树（实时更新）
                ```mermaid
                graph TB
                ...
                🎯 主力行为跟踪表
                时间窗口	异常信号	对应操作	成功率
                10:00-10:30	万手大单连续吃筹（＞3笔）	▶ 跟随抢筹 ≤15%仓位	82%
                13:00-13:30	盘口突现9999手压单	▶ 提前挂单砸穿压单价位（低0.5%）	76%
                14:30-15:00	北向资金反向异动（±1亿）	▶ 同步调仓（跟买/跟卖）	68%

                💡 分时战术工具箱
                战法名称	触发条件	指令示例
                缺口引力战	未补缺口±1%波动	▶ ￥10.20挂单买（预填缺口）
                T+0 套利术	振幅＞5% & MACD金叉	▶ 底仓不动，浮动仓做波段（买低卖高）
                尾盘突袭侦测	14:45突然放量（＞日均200%）	▶ 龙虎榜预判股 → 抢筹跟涨≤10%仓位

                📉 动态风控引擎（全时段生效）
                ⚡ 黑天鹅应急协议
                1. **政策利空突袭**（如集采降价）：  
                - 立即砍仓至50% → 挂跌停价卖出剩余仓位（避免流动性枯竭）  

                2. **个股闪崩预警**（15分钟跌＞8%）：  
                - 检查融资余额：若担保比＜130% → 断头铡风险 → 清仓  
                - 若属错杀：反抽分时均线时减仓70%  

                3. **大盘系统性风险**（沪深300跌＞3%）：  
                - 启动对冲：买入50ETF认沽期权（行权价-5%）  
                - 个股仓位压缩至30%  

                📌 仓位动态平衡公式
                当日最大仓位 = 基础仓位 × [1 + (情绪系数 - 0.5)] × 技术强度系数  
                基础仓位：30%（默认）
                情绪系数：涨停家数/总交易股数（＞0.6激进，＜0.4谨慎）
                技术系数：
                多头排列：1.2
                箱体震荡：1.0
                空头趋势：0.6
                ...
                🌙 盘后总结与明日计划
                📈 今日操作复盘：高开低走未触买点，观望为主
                🎯 明日关键点：支撑￥9.80/压力￥10.50，变盘窗口10:30      
                💰 仓位调整：主力净流出2.1亿，明日仓位上限50%
